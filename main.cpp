...
#include "src/OpenWildstacksRequests.h"
...
public:
    ...
    static void exitHandler(int)
    {
        ...
        s_shouldExit = true;
        OMINFO << "Request to finish the openwildstacks received";
        cv.notify_one();
    }
...
auto wildstacks_log_level  =
        *(opts.get_option<size_t>("wildstacks-log-level"));

auto verbose_level = 
        *(opts.get_option<size_t>("verbose"));

if (wildstacks_log_level < 1 || wildstacks_log_level > 4)
{
    cerr << "wildstacks-log-level,m option must be between 1 and 4!\n";
    return EXIT_SUCCESS;
}
...
// setup wildstacks logger
mlog_configure(mlog_get_default_log_path(""), true);
mlog_set_log(std::to_string(wildstacks_log_level).c_str());

auto log_file  = *(opts.get_option<string>("log-file"));

// setup a logger for Open wildstacks
...
el::Loggers::reconfigureLogger(OPENWILDSTACKS_LOG_CATEGORY, defaultConf);

OMINFO << "OpenWildstacks is starting";
...
// with the wildstacks deamon.
if (!current_bc_status->init_wildstacks_blockchain())
{
...
xmreg::OpenWildstacksRequests open_wildstacks(mysql_accounts, current_bc_status);

// create Open WildStacks APIs
MAKE_RESOURCE(login);
MAKE_RESOURCE(ping);
...
// Publish the Open WildStacks API created so that front end can use it
