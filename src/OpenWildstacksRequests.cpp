...
#include "OpenWildstacksRequests.h"
...
OpenWildstacksRequests::OpenWildstacksRequests(
...
void
OpenWildstacksRequests::login(const shared_ptr<Session> session, const Bytes & body)
{
    json j_response;
    json j_request;
...
void
OpenWildstacksRequests::ping(const shared_ptr<Session> session, const Bytes & body)
{
    json j_response;
    json j_request;
...
void
OpenWildstacksRequests::get_address_txs(
        const shared_ptr< Session > session, const Bytes & body)
{
    ...
    j_response = json {
            ...
            {"scanned_height"         , 0},    // not used. just to match mywildstacks
...
void
OpenWildstacksRequests::get_address_info(
        const shared_ptr< Session > session, const Bytes & body)
{
    json j_response;
    json j_request;
    ...
    j_response = json {
            ...
            {"scanned_height"         , 0},    // not used. it is here to match mywildstacks
            {"scanned_block_height"   , 0},    // taken from Accounts table
            {"scanned_block_timestamp", 0},    // taken from Accounts table
            {"start_height"           , 0},    // not used, but available in Accounts table.
                                               // it is here to match mywildstacks
            {"blockchain_height"      , 0},    // current blockchain height
...
void
OpenWildstacksRequests::get_unspent_outs(
...
void
OpenWildstacksRequests::get_random_outs(
        const shared_ptr< Session > session, const Bytes & body)
{
    json j_request;
    json j_response;
    ...
    else
    {
        ...
        j_response["error"]  = "Error getting random "
                               "outputs from wildstacks deamon";
    }
...
void
OpenWildstacksRequests::submit_raw_tx(
        const shared_ptr< Session > session, const Bytes & body)
{
...
// handled in mywildstacks-app-js
void
OpenWildstacksRequests::import_wallet_request(
        const shared_ptr< Session > session, const Bytes & body)
{
...
void
OpenWildstacksRequests::import_recent_wallet_request(
        const shared_ptr< Session > session, const Bytes & body)
{
    json j_response;
    json j_request;
...
void
OpenWildstacksRequests::get_version(
        const shared_ptr< Session > session,
        const Bytes & body)
{
    ...
    json j_response {
        ...
        {"wildstacks_version_full" , string {WILDSTACKS_VERSION_FULL}},
        {"api"                 , OPENWILDSTACKS_RPC_VERSION},
...
shared_ptr<Resource>
OpenWildstacksRequests::make_resource(
        function< void (OpenWildstacksRequests&, const shared_ptr< Session >,
...
void
OpenWildstacksRequests::generic_options_handler(
        const shared_ptr< Session > session )
{
    const auto request = session->get_request( );
...
OpenWildstacksRequests::make_headers(
...
void
OpenWildstacksRequests::print_json_log(const string& text, const json& j)
{
    cout << text << '\n' << j.dump(4) << endl;
}
...
string
OpenWildstacksRequests::body_to_string(const Bytes & body)
{
    return string(reinterpret_cast<const char *>(body.data()), body.size());
}

json
OpenWildstacksRequests::body_to_json(const Bytes & body)
{
...
uint64_t
OpenWildstacksRequests::get_current_blockchain_height() const
{
    return current_bc_status->get_current_blockchain_height();
}

bool
OpenWildstacksRequests::login_and_start_search_thread(
...
bool
OpenWildstacksRequests::parse_request(
        const Bytes& body,
        ...
        json& j_response) const
{
    ...
    {
        cerr << "OpenWildstacksRequests::parse_request: " << e.what() << endl;
...
OpenWildstacksRequests::create_account(
        ...
        bool generated_locally) const
{
    ...
    //how mywildstacks is doing this. At the momemnt, I'm not sure.
...
boost::optional<XmrAccount>
OpenWildstacksRequests::select_account(
        string const& xmr_address,
        string const& view_key,
...
bool 
OpenWildstacksRequests::make_search_thread(
...
OpenWildstacksRequests::select_payment(
...
void
OpenWildstacksRequests::session_close(
        const shared_ptr< Session > session,
        json& j_response,
        int return_code,
