...
namespace xmreg
{
    ...
    CmdLineOptions::CmdLineOptions(int acc, const char *avv[]) {
        ...
        options_description desc(
                "openwildstacks, Open WildStacks backend service");
        ...
        desc.add_options()
                ...
                ("port,p", value<string>()->default_value("1984"),
                 "default port for restbed service of Open WildStacks")
                ("config-file,c", value<string>()
                 ->default_value("./config/config.json"),
                 "Config file path.")
                ("wildstacks-log-level,m", value<size_t>()
                 ->default_value(1),
                 "WildStacks log level 1-4, default is 1.")
                ("verbose,v", value<size_t>()
                 ->default_value(0),
                 "OpenWildStacks log verbose level 0-4, default is 0.")
                ("log-file,l", value<string>()
                 ->default_value("./openwildstacks.log"),
