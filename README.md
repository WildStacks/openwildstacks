# OpenWildstacks - a fully open sourced implementation of MyWildstacks backend

In this example [restbed](https://github.com/Corvusoft/restbed/) is used to
demonstrate how to provide WildStacks related JSON REST service. For this purpose,
 a service called Open WildStacks was developed.


OpenWildstacks is an open source implementation of backend of MyWildstacks

Open WildStacks's backend is open sourced, free
to use, host and modify. Additionally, some features were added/changed as compared
to MyWildStacks. They include:

 - google analytics, cloudflare, images and flash were removed.
 - frontend's html and javascript upgrades and cleanups.
 - transaction service fee was set to zero (same as in MyWildStacks).
 - the wallets generated use 25 word mnemonics, fully compatible with official wildstacks wallets
(13 word mnemonics generated by MyWildStacks work as usual though).
...
The Open WildStacks consists of four components that need to be setup for it to work:
 ...
 - Frontend - it is virtually same as that of MyWildStacks, except before mentioned differences.
  It consists of HTML, CSS, and JavaScript.
 - WildStacks daemon - daemon must be running and fully sync, as this is
 where all transaction data is fetched from and used. Daemon also commits txs
 from the Open WildStacks into the WildStacks network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses WildStacks blockchain and "talks"
 with WildStacks deamon.
...
Open WildStacks is not as fast as MyWildStacks.
...
Below are example and basic instructions on how to setup up and run Open WildStacks on Ubuntu 16.04.
For other Linux operating systems, the instructions are analogical.


#### WildStacks download and compilation

To download and compile recent WildStacks follow instructions
in the following link:

https://github.com/wildstacks/wildstacks/blob/master/README.md

#### Compilation of the OpenWildStacks (don't run it yet)

Once WildStacks was downloaded and compiled, we can download Open WildStacks and compile it.
...
work without database, setup frontend, and synced and running wildstacks blockchain.
...
git clone --recursive https://github.com/wildstacks/openwildstacks.git
...
# altearnatively can use cmake -DWILDSTACKS_DIR=/path/to/wildstacks_folder ..
...
Create openwildstacks database called `openwildstacks`.

```
cd openwildstacks/sql
docker exec -i ommariadb mysql -uroot -proot < openwildstacks.sql
```

#### PhpMyAdmin (using docker)
A good way to manage/view the openwildstacks database is through the
...
```
docker run --name omhtml -p 80:80 -v /home/mwo/openwildstacks/html:/usr/share/nginx/html:ro -d nginx
```

where `omhtml` is docker container name, `80:80` will expose the frontend
on port 80 of the localhost, and `/home/mwo/openwildstacks/html` is the location on your host computer where the
...
openwildstacks, Open WildStacks backend service:
  ...
  -c [ --config-file ] arg (=./config/config.json)
                                        Config file path.
  -m [ --wildstacks-log-level ] arg (=1)    WildStacks log level 1-4, default is 1.
  -l [ --log-file ] arg (=./openwildstacks.log)
...
Before running `openwildstacks`:
 ...
 - make sure wildstacks daemon is running and fully sync. If using testnet or stagenet networks, use wildstacks daemon
...
```bash
./openwildstacks -s -c /path/to/config.json
```


## OpenWildStacks JSON REST API

Example JSON REST requests and their responses of [OpenWildStacks](https://github.com/wildstacks/openwildstacks) are provided below. The long term goal
...
the OpenWildStacks api does not fully conform to that.

#### get_version

Get version of the OpenWildStacks, its API and wildstacks.
...
```json
{
  ...
  "wildstacks_version_full": "0.10.3.1-125f823"
}
```
...
Login an existing or a new user into OpenWildStacks.
...
Get random outputs from the wildstacks daemon to be used as ring members
...
Submit raw tx in hex generated by the fronted to be relayed to wildstacks network.
